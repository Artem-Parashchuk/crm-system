...
export function extractStringValue(value: unknown): string | null {
  ...
  const candidates = [record.name, record.title, record.companyName, record.customerName]
  ...
  if (company && typeof company === 'object') {
    const companyName = extractStringValue((company as Record<string, unknown>).name)
    if (companyName) {
      return companyName
    }
  }
  ...
  if (customer && typeof customer === 'object') {
    const customerName = extractStringValue((customer as Record<string, unknown>).name)
    if (customerName) {
      return customerName
    }
  }
...
export async function buildCustomerNameMap(
  ...
  try {
    ...
    for (const doc of result.documents) {
      ...
      if (id) {
        const name =
          extractStringValue(record.name) || extractStringValue(record.companyName) || '—'
        map[id] = name
      }
    }
    return map
...
export function getCompanyName(deal: IDeal, customerNameMap?: Record<string, string>): string {
  if (typeof deal.customer === 'object' && deal.customer !== null) {
    const name = extractStringValue((deal.customer as Record<string, unknown>).name)
    if (name) return name
  }
